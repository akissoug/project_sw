...
class PowerMonitorMultiMission(Node):
    def __init__(self):
        ...
        self.declare_parameter('battery_capacity_mah', 10000.0)
        ...
        self.last_landing_position = None  # Store where we landed
    ...
    def gps_callback(self, msg):
        """Process GPS with multi-mission home position updates"""
        with self.lock:
            self.global_position = msg
            
            # Set/update home position after each arm
            if (self.is_armed() and
                msg.lat != 0 and msg.lon != 0):
                
                # Set new home if we don't have one or if we've moved significantly
                should_update_home = False
                
                if self.home_position is None:
                    should_update_home = True
                    self.get_logger().info(f'🏠 Setting initial home position')
                else:
                    # Check if we've taken off from a different location
                    if self.last_landing_position is not None:
                        lat_diff = abs(msg.lat - self.last_landing_position[0])
                        lon_diff = abs(msg.lon - self.last_landing_position[1])
                        # If we're more than ~50m from last landing, update home
                        if lat_diff > 0.0005 or lon_diff > 0.0005:
                            should_update_home = True
                            self.get_logger().info(f'🏠 Updating home - new takeoff location detected')
                
                if should_update_home:
                    self.home_position = (msg.lat, msg.lon, msg.alt)
                    self.get_logger().info(
                        f'🏠 Home position {"updated" if self.mission_count > 1 else "set"}: '
            ...
            if self.is_armed() and hasattr(msg, 'alt'):
                ...
                if altitude_agl is not None:
                    ...
                    if self.in_wig_effect and not was_in_wig:
                        ...
                        self.get_logger().info(
                            f'🌊 Entered WIG effect - Alt: {altitude_agl:.1f}m, '
                            f'h/b: {self.current_h_over_b:.2f}, '
                            f'Efficiency: +{self.current_wig_efficiency*100:.1f}% (INFO)'
                        )
    ...
    def status_callback(self, msg):
        ...
        with self.lock:
            ...
            if previous_mode != current_mode:
                ...
                # Landing detection
                if current_mode == 'AUTO.LAND' or (previous_mode == 'AUTO.LAND' and current_mode != 'AUTO.LAND'):
                    if self.global_position:
                        self.last_landing_position = (
                            self.global_position.lat,
                            self.global_position.lon,
                            self.global_position.alt
                        )
                        self.get_logger().info(f'🛬 Landing position recorded')
    ...
    def calculate_remaining_time(self):
        """Calculate battery time remaining (conservative)"""
    ...
    def handle_disarm(self):
        ...
        # Store disarm time but don't reset session
        self.last_disarm_time = time.time()
        self.armed_time = None
        self.mission_start_time = None
        
        # Don't reset home here - keep it for next mission reference
        # Don't reset battery history - keep it for session continuity
    ...
    def check_battery_status(self):
        ...
        with self.lock:
            ...
            if altitude:
                ...
                if self.in_wig_effect:
                    log_parts.append(f'🌊 WIG: +{self.current_wig_efficiency*100:.0f}%')
